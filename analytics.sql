...
Select season, avg("stolenBases") from mlb_hitters where "atBats" > 100 group by season order by season;
...
Select season, avg("balks") from mlb_pitchers where "inningsPitched" > 30 group by season order by season;

Select season, avg("strikeoutWalkRatio"::numeric) from mlb_pitchers where "inningsPitched" > 30 group by season order by season;

Select season, avg("baseOnBalls") from mlb_hitters where "atBats" > 30  group by season order by season;

Select season, avg("hitByPitch") from mlb_pitchers where "inningsPitched" > 30 group by season order by season;
